...
class MapboxNavigationView extends StatelessWidget {
  ...
  // New Event Channel
  static const EventChannel _mapboxViewChannel = EventChannel('mapboxView/events');
  ...
  starNavigation() async {
    await _navigationWithMapboxPlugin.startNavigation(
        ...
        origin: WayPoint(latitude: mapboxOptions.origin.latitude, longitude: mapboxOptions.origin.longitude),
        // destination refers to the end point or goal for the user at the time of starting the navigation
        destination: WayPoint(latitude: mapboxOptions.destination.latitude, longitude: mapboxOptions.destination.longitude),
        ...
        voiceUnits: (mapboxOptions.voiceUnits != null) ? mapboxOptions.voiceUnits : '',
        // language in which the navigation assistant will speak to us
        // optional, default: en
        language: (mapboxOptions.language != null) ? mapboxOptions.language : '',
  ...
  Widget build(BuildContext context) {
    Map data = {
      'destination': {'Latitude': mapboxOptions.destination.latitude, 'Longitude': mapboxOptions.destination.longitude},
      'origin': {'Latitude': mapboxOptions.origin.latitude, 'Longitude': mapboxOptions.origin.longitude},
      'simulateRoute': mapboxOptions.simulateRoute,
      'setDestinationWithLongTap': mapboxOptions.setDestinationWithLongTap,
      // 'msg': (mapboxOptions.msg != null) ? mapboxOptions.msg : '',
      'profile': (mapboxOptions.profile != null) ? mapboxOptions.profile : '',
